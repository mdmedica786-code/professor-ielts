...
import { task2Data as DATA } from '../../data/masterclassTask2Data';
...
const DEFAULT_BAND_1 = '7.5';
const DEFAULT_BAND_2 = '9.0';

export default function Task2Masterclass() {
    const [currentVisual, setCurrentVisual] = useState('opinion');
    const [currentScenario, setCurrentScenario] = useState(0);
    const [isCompare, setIsCompare] = useState(false);
    const [band1, setBand1] = useState(DEFAULT_BAND_1);
    const [band2, setBand2] = useState(DEFAULT_BAND_2);
    const [modalData, setModalData] = useState(null); // { title, body, footer }
        ...
        <div className="flex h-full w-full bg-slate-50 relative overflow-hidden">
            ...
            <div className="w-64 bg-slate-900 text-slate-300 flex flex-col flex-shrink-0">
                <div className="p-6 border-b border-slate-800">
                    <h1 className="text-xl font-bold text-white tracking-tight">Task 2</h1>
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                            ...
                            <div id="diagnostics-content" className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4 hidden">
                                <div className="p-4 rounded-lg bg-indigo-50/50 border border-indigo-100/50">
                                    <div className="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-1">Topic</div>
                                    <div className="text-sm text-slate-700">{scenario.diagnostics.topic}</div>
                                </div>
                                <div className="p-4 rounded-lg bg-emerald-50/50 border border-emerald-100/50">
                                    <div className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-1">Issue</div>
                                    <div className="text-sm text-slate-700">{scenario.diagnostics.issue}</div>
                                </div>
                                <div className="p-4 rounded-lg bg-amber-50/50 border border-amber-100/50">
                                    <div className="text-xs font-bold text-amber-600 uppercase tracking-wider mb-1">Task Requirement</div>
                                    <div className="text-sm text-slate-700">{scenario.diagnostics.task_req}</div>
                                </div>
                                <div className="p-4 rounded-lg bg-rose-50/50 border border-rose-100/50">
                                    <div className="text-xs font-bold text-rose-600 uppercase tracking-wider mb-1">Brainstorming</div>
                                    <div className="text-sm text-slate-700">{scenario.diagnostics.brainstorm}</div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div id="vocab-content" className="p-6 hidden">
                                <div className="flex flex-wrap gap-2">
                                    {scenario.vocabulary.map((v, i) => (
                                        <span key={i} className="inline-flex items-center px-3 py-1.5 rounded-md text-sm font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-full overflow-hidden">
            ...
            <div className="bg-slate-50 border-t border-slate-100 p-4">
                <div className="flex flex-wrap gap-2 text-xs font-medium">
                    ...
                    <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-rose-500"></span>Task Response</span>
                </div>
            </div>
        </div>
    );
